...
SetJSONValue = tuple[str]
SetValue = tuple[Pubkey]
...
RuleSetToggleKind = typing.Union[None_, Clear, Set]
RuleSetToggleJSON = typing.Union[NoneJSON, ClearJSON, SetJSON]


def from_decoded(obj: dict) -> RuleSetToggleKind:
    ...
    if "Set" in obj:
        val = obj["Set"]
        return Set((val["item_0"],))
    raise ValueError("Invalid enum object")


def from_json(obj: RuleSetToggleJSON) -> RuleSetToggleKind:
    if obj["kind"] == "None":
        return None_()
    if obj["kind"] == "Clear":
        return Clear()
    if obj["kind"] == "Set":
        ...
        return Set((Pubkey.from_string(set_json_value[0]),))
    kind = obj["kind"]
...
layout = EnumForCodegen(
    "None" / borsh.CStruct(),
    "Clear" / borsh.CStruct(),
    "Set" / borsh.CStruct("item_0" / BorshPubkey),
)
