...
class UnlockArgs(typing.TypedDict):
    unlock_args: types.unlock_args.UnlockArgsKind


layout = borsh.CStruct("unlock_args" / types.unlock_args.layout)


class UnlockAccounts(typing.TypedDict):
    authority: Pubkey
...
def unlock(
    ...
    identifier = b"e\x9b(\x15\x9e\xbd8\xcb"
    encoded_args = layout.build(
        {
            "unlock_args": args["unlock_args"].to_encodable(),
        }
    )
